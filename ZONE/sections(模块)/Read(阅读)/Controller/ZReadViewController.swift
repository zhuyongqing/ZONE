...
class ZReadViewController: ZBaseViewController,UITableViewDelegate,UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        var cell:ZReadTableCell? = tableView.dequeueReusableCell(withIdentifier: cellID) as? ZReadTableCell;
        if (cell == nil) {
            cell = ZReadTableCell.init(style: .default, reuseIdentifier: cellID);
        }
        
        cell?.readModel = self.datas[indexPath.row];
        cell?.getCellHeight();
        
        return (cell?.readModel?.cellHeight)!;
    }
